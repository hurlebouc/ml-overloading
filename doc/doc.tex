...
\begin{itemize}
	...
	\item Le deuxième permet de savoir quelles sont les règles associées aux 
		...
		de $\dn$. Ici, ont ne fait pas la différence entre le nom des variables
		...
		de type comme indépendantes les unes des autre. Par exemple, le type 
...
\begin{itemize}
	...
	\item Si $\tau = \tau_1 \rightarrow \tau_2$ et que le troisième champs de $\dn$
		est $(\dn_1, \dn_2)$ alors
		...
		Si le troisième champs de $\dn$ a la valeur \texttt{None}, alors 
		$$ \Sigma^\tau_\dn = \Gamma_\dn.$$
	\item Si $\tau = (\tau_1, ..., \tau_n)\epsilon$ et que le quatrième champs de 
		$\dn$ contient $\epsilon$ auquel est associé $(\dn_1, ..., \dn_n)$, alors 
		$$ \Sigma^\tau_\dn = \Gamma_\dn \cup \bigcap_{i=1}^{n} \Sigma^{\tau_i}_{\dn_i} $$

		Si $\epsilon$ n'appartient pas au dernier champs de $\dn$, alors 
...
du deuxième champs de $\dn$ : en effet, ce champs représente l'ensemble des règles "sous
...
proportionnel \textit{en théorie} à taille du codomaine. Mais en pratique, ce 
résultat peut être faussé par l'implémentation que l'on a fait des ensembles de
règle : en effet, on a choisi d'utiliser des listes pour représenter les 
...
flèche par exemple.}. De plus, les intersections entre deux ensembles de taille
$n$ et $m$ se sont en complexité proportionnelle à $nm$.
